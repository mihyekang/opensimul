"""analyze-monthly / analyze-weekly 엔드포인트 단위 테스트."""
import json
import pytest
...
ITEMS_WEEK1 = [
    {"raw_name": "사과", "amount": 3000, "purchase_date": "2024-02-12",
     "merchant": "마트A", "receipt_id": 1, "qty": 1, "unit_price": 3000},
]

ITEMS_WEEK2 = [
    {"raw_name": "계란", "amount": 5000, "purchase_date": "2024-02-05",
     "merchant": "마트A", "receipt_id": 2, "qty": 1, "unit_price": 5000},
]

AI_ONE_WEEK = json.dumps({
    "summary": "이번 주 신선식품 위주 소비",
    "weeks": [
        {
            "label": "이번 주", "period": "2024-02-12 ~ 2024-02-18", "total": 3000,
            "categories": {
                "신선식품": 3000, "가공식품": 0, "음료/주류": 0,
                "생활용품": 0, "반려동물용품": 0, "기타": 0,
            },
        }
    ],
...
AI_TWO_WEEKS = json.dumps({
    "summary": "이번 주가 지난 주보다 소비 감소",
    "weeks": [
        {
            "label": "이번 주", "period": "2024-02-12 ~ 2024-02-18", "total": 3000,
            "categories": {
        ...
        {
            "label": "지난 주", "period": "2024-02-05 ~ 2024-02-11", "total": 5000,
            "categories": {
                "신선식품": 5000, "가공식품": 0, "음료/주류": 0,
                "생활용품": 0, "반려동물용품": 0, "기타": 0,
            },
...
class TestAnalyzeWeeklyEmpty:
    def test_empty_items_returns_empty(self, authed_client):
        """DB에 아이템 없으면 AI 호출 없이 빈 응답 반환."""
        with patch("db.get_items_by_date_range", return_value=[]):
            resp = authed_client.post(
                "/grocery/analyze-weekly",
                json={"start_date": "2024-02-01", "end_date": "2024-02-18"},
            )
        assert resp.status_code == 200
    ...
    def test_invalid_date_returns_400(self, authed_client):
        ...
        resp = authed_client.post(
            "/grocery/analyze-weekly",
            json={"start_date": "bad-date", "end_date": "2024-02-18"},
        )
        assert resp.status_code == 400


class TestAnalyzeWeeklyOneWeek:
    def test_single_week_returns_one_entry(self, authed_client):
        """한 주치 데이터 → weeks 길이 1, label '이번 주'."""
        mock_client = MagicMock()
        mock_client.chat.return_value = (AI_ONE_WEEK, {})

        with patch("db.get_items_by_date_range", return_value=ITEMS_WEEK1), \
             patch("app._client", return_value=mock_client):
            resp = authed_client.post(
                "/grocery/analyze-weekly",
                json={"start_date": "2024-02-12", "end_date": "2024-02-18"},
            )

        assert resp.status_code == 200
        data = resp.json()
        assert len(data["weeks"]) == 1
        assert data["weeks"][0]["label"] == "이번 주"
        assert data["summary"] != ""


class TestAnalyzeWeeklyTwoWeeks:
    def test_two_weeks_returns_two_entries(self, authed_client):
        """두 주치 데이터 → weeks 길이 2, 최신 주 먼저."""
        mock_client = MagicMock()
        mock_client.chat.return_value = (AI_TWO_WEEKS, {})

        with patch("db.get_items_by_date_range", return_value=ITEMS_WEEK1 + ITEMS_WEEK2), \
             patch("app._client", return_value=mock_client):
            resp = authed_client.post(
                "/grocery/analyze-weekly",
                json={"start_date": "2024-02-05", "end_date": "2024-02-18"},
            )
        ...
        assert len(data["weeks"]) == 2
        assert data["weeks"][0]["label"] == "이번 주"
        assert data["weeks"][1]["label"] == "지난 주"

    def test_ai_parse_failure_falls_back_to_raw_text(self, authed_client):
        """AI가 유효하지 않은 JSON 반환 → summary는 raw 텍스트 일부, weeks=[]."""
        mock_client = MagicMock()
        mock_client.chat.return_value = ("분석 결과를 생성할 수 없습니다.", {})
